...
def print_xy_menu(
    *,
    ...
    # Hide offset / y-range / derivative in stack mode (dispatch also blocks these).
    if stack:
        col2 = [
            item for item in col2
            if not item.startswith("o:")
            and not item.startswith("y:")
            and not item.startswith("d:")
        ]

    if not is_diffraction:
        # Crosshair Q/d helpers and XRD-only axis-unit convert
        col3 = [
            item for item in col3
            if not item.startswith("n:") and not item.startswith("u:")
        ]
    print_menu_columns(
