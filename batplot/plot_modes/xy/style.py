"""XY style helpers: print diagnostics and export/import style configs.

This module owns the 1D/XY .bps/.bpsg style implementation. The root
``batplot.style`` module remains a compatibility shim for existing imports.
"""
...
def _grid_enabled(ax) -> bool:
    """Return True if any major gridline is currently visible."""
    try:
...
def capture_xy_axis_style(ax) -> Dict[str, Any]:
    """Capture tick/label colors and labelpads for XY p/i/s/b parity."""
    return {
        "tick_colors": {
        ...
        "labelpads": {
            "x": float(getattr(ax.xaxis, "labelpad", 0.0) or 0.0),
            "y": float(getattr(ax.yaxis, "labelpad", 0.0) or 0.0),
        },
    }


def apply_xy_axis_style(
    ax,
    style: Optional[Dict[str, Any]],
    *,
    fig=None,
    spines_cfg: Optional[Dict[str, Any]] = None,
) -> None:
    """Restore tick/label colors and labelpads (skips legacy colors when spines are per-side)."""
    if not style:
        return
    spines_cfg = spines_cfg or {}
    has_per_spine_color = any(
        isinstance(sp, dict) and sp.get("color") for sp in spines_cfg.values()
    )
    if not has_per_spine_color:
        tick_colors = style.get("tick_colors") or {}
        if tick_colors:
            try:
                if "x" in tick_colors:
                ...
                pass
    axis_label_colors = style.get("axis_label_colors") or {}
    if axis_label_colors:
        try:
            ...
            pass
    labelpads = style.get("labelpads") or {}
    try:
        if labelpads.get("x") is not None:
            ax._pending_xlabelpad = float(labelpads["x"])
        if labelpads.get("y") is not None:
            ax._pending_ylabelpad = float(labelpads["y"])
    except Exception:
        pass


def serialize_curve_palette_history(fig) -> List[Dict[str, Any]]:
    """Serialize ``fig._curve_palette_history`` for .bpsg export and .pkl sessions."""
    history = getattr(fig, '_curve_palette_history', None) or []
...
def _resolve_palette_cmap(palette_name: str):
    """Resolve a palette name to a matplotlib Colormap (all matplotlib versions)."""
    return get_colormap(palette_name)
...
def export_style_config(
    ...
    try:
        ...
        cfg = {
            ...
            "grid": _grid_enabled(ax),
            "lines": [
                {
                    "index": i,
                    "label": labels[i] if i < len(labels) else ln.get_label(),
...
def apply_style_config(  # pyright: ignore[reportGeneralTypeIssues] - too complex for full analysis
    filename: str,
    fig,
    ax,
    ...
    # Lines (support dual y-axis: fig._xy_lines_by_curve when --ry)
        ...
        for entry in cfg.get("lines", []):
            ...
            if "color" in entry and entry["color"] is not None:
                apply_curve_color(ln, entry["color"])
            else:
            ...
            if "label" in entry:
                try:
                    new_label = str(entry.get("label") or "")
                    if idx < len(labels):
                        labels[idx] = new_label
                    ln.set_label(new_label)
        ...
        if "show_cif_hkl" in cfg:
            try:
                ...
                fig._bp_show_cif_hkl = bool(cfg["show_cif_hkl"])
    ...
    # Axis title toggle state
        try:
            ...
            # Prefer labelpads from the style file; fall back to pre-import ax pads
